...
uint8_t IAP_Main_Deal( void )
{
    ...
    if( (CHRV3DiskStatus >= DISK_MOUNTED) && op_flag )
    {
        ...
        else
        {
            ...
            if ( strcmp(extention, ".R32") == 0) cartType = ROM32k;
            if ( strcmp(extention, ".KON") == 0) cartType = KonamiWithoutSCC;
            if ( strcmp(extention, ".SCC") == 0) cartType = KonamiWithSCC;
            if ( strcmp(extention, ".A8K") == 0) cartType = ASCII8k;
            if ( strcmp(extention, ".A16") == 0) cartType = ASCII16k;
